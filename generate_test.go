...
func TestRenderCatalogStructure(t *testing.T) {
	...
	// 2 planyolo × 2 fallback. Without this a model dial that quietly lost
	// "elite" — or a generator that stopped emitting elite combos on the lanes
	// that can host them — would pass silently.
	if want := 2 * 6 * 4 * 2 * 2 * 2 * 2 * 2; eliteSeen != want {
	...
	for _, key := range []string{"prewalk", "planyolo", "advisor", "fast", "fallback"} {
	...
	// Nor the catalog: the fallback dial drops the chains from the overlay at
	// launch, and never asks the generator for a chainless block. Every combo
	// id is <lane>_<mtier>_<thinking>_<sp|nosp> and every block still says
	// "fallback on", because that is what the block carries.
	for _, l := range strings.Split(out, "\n") {
		if l == "" || strings.HasPrefix(l, "__") {
			continue
		}
		if l[0] != ' ' && strings.Contains(l, "fallback") {
			t.Errorf("a combo id carries the fallback dial: %q", l)
		}
		if strings.Contains(l, "fallback off") {
			t.Errorf("a catalog row was generated chainless: %q", l)
		}
	}
}
