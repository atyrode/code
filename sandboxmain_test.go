...
//
// The other is a stand-in OMP. A contained session has no shell, so a fake
// OMP has to be a real executable rather than a script, and the only real
// executable a test can put inside the boundary is this one. It is selected
// by environment rather than by argument, because the launch owns OMP's
// command line on purpose and a test that could inject an argument into it
// would be exercising a command line production never builds.
