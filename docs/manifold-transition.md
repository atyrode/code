...
| 1 — plugin resources and operations | proven | On 2026-09-13 the integration pinned Manifold `c731aa2364531ec6cf14e6ffb13c3a806dff9f6b` and OMP `19223eb4d08ad8aef70b31fb8ff9d716d91e597b`; all three OMP and four Code bundles packed and verified, and the exact OMP root bundle `adb28f4090de70deb3a8b5d9cf36bc2ce2fd53ae21a41624f000bc35a5236e71` reached ready on both preview destinations through native review/apply. |
| 2 — web-controlled launch | proven | On 2026-09-13 the real Code workflow composed the shared profile and saved account pool, rechecked its destination-bound native review, and opened account-backed OMP terminals with output on `dev-01` and `Code isolated destination`. No terminal-input control channel or destination substitution was used. |
| 3 — native workspace and multiplayer | proven | On 2026-09-13 two independent browser identities converged through the container CAS while viewer authority remained read-only; prompts, drafts and visited views survived destination changes. Native validation exited 0 for a real workspace on both separately enrolled destinations, and the actual panel distinguished both online destinations from an offline one. |
| 4 — accounts and usage | proven | On 2026-09-13 OMP assumed single-owner custody of the preserved nine-account store without reauthentication or credential copying. Account-backed launches succeeded on both destinations, existing Code/Usage clients survived the transfer and restart, and a legacy client completed a fresh quota observation. Run-scoped credentials and grants were revoked; existing backups remain. |
| 5 — catalog, suggestions and first use | proven | On 2026-09-13 both preview destinations produced readable 11-model inventories from the complete saved account pool after native workspace validation. First-use, permission review and refusal paths passed the two-browser gate; the actual preview panel withheld the suggestion control because no classifier service is configured rather than presenting unavailable behavior. |
| 6 — retire deprecated implementation | proven | #153 removes the Go/CLI/Nix/state/runtime-worker surface and obsolete contracts after explicit reachability checks, with no compatibility shim. Its final Bun 1.4.2 gate passed 87 tests / 530 assertions plus all seven bundle lifecycle checks and the two-browser verifier; preview acceptance above completed before merge. |

Draft #148's prototype commit
`381ac912707e1ccc2b6ecbf8f4dcc75c8e99ab17` and the 2026-09-09 five-Code-bundle
preview apply are historical only. #153 is the verified cross-repository
cutover recorded above. Release tagging, production installation, backup
disposal and other operational authority remain separate from source merge and
from this preview proof.
