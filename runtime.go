...
func runRuntimeTarget(target, thinking, prompt, dir string, forwarded []string) int {
	path, err := resolveLaunchPath("CODE_RUNTIME_BROKER", nil)
	if err != nil {
		fmt.Fprintln(os.Stderr, "code: runtime broker not found:", err)
		return 1
	}
	err = runChild(path, runtimeLaunchArgv(path, target, thinking, forwarded, prompt), withoutAuthEnv(os.Environ()), dir)
	if err != nil {
