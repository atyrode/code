...
func loadCatalogBytes(raw []byte, path string) (*catalog, error) {
	if err := validateModelsYAML(raw); err != nil { return nil, err }
	var doc yaml.Node
	...
	// Only a reviewed, probe-backed native catalog is eligible for routing.
	if probedNode == nil || probedNode.Value != "true" {
		return nil, fmt.Errorf("%s: models are not probe-verified", path)
	}
	ladder := map[string][5]string{}
