...
// just model/thinking/advisor. lane and the spark/fast toggles are left to
...
// the spark facet can only run on a lane whose pool-set contains its provider's
// pool, and it cannot be left on when its quota bucket is maxed or unauthed.
// Runs after an applied proposal, so the generator can't land on an impossible
// or unavailable combo.
...
// deriveToggles then sets spark/fast from the resulting sizing (the 3B
...
// suggestion applied: the model's direct picks plus the derived spark/fast
...
// deriveToggles sets the spark/fast toggles from the suggested sizing plus
...
//
// The capability ladder itself is the classifier's own pick (the model facet),
// so nothing is derived for it here.
...
// spending the pay-as-you-go pool: below it, its led lanes are not proposed.
