...
export function CatalogWorkbench({ host, target, available, onDone }: { host: HostServices; target: Target | null; available: boolean; onDone: () => void }) {
  ...
  return <section className="plugin-atyrode_code_generator__catalog" aria-label="Model catalog">
    ...
    {!review && !editor && <div className="plugin-atyrode_code__toolbar">
      <button type="button" className={!record?.active ? "plugin-atyrode_code__primary-action" : undefined} data-action="atyrode.omp.startInventory" disabled={!writable || busy || !available || !inventoryReady || inventoryRunning} onClick={() => { if (record && target) void perform(async () => { const job = await codeWorkflow(host).startInventory(target, record.revision); if (mounted.current) setInventoryId(job.jobId); }); }}>{inventoryRunning ? "Discovering models…" : record?.active ? "Refresh model inventory" : "Discover models"}</button>
    ...
    {inventoryId && <div className="plugin-atyrode_code_generator__job-progress" role="status">
      ...
      {inventory.data && !editor && !review && <button type="button" className="plugin-atyrode_code__primary-action" data-action="atyrode.code.stageCatalog" disabled={!writable || busy} onClick={() => { if (record && inventory.data) void perform(() => stage(inventory.data!.draft.document, record.revision)); }}>Review these models</button>}
    ...
    {editor && <div className="plugin-atyrode_code_generator__model-editor">
      ...
      <div className="plugin-atyrode_code__toolbar"><button type="button" className="plugin-atyrode_code__primary-action" data-action="atyrode.code.stageCatalog" disabled={busy || stale || !parsed?.success} onClick={() => { if (parsed?.success) void perform(() => stage(parsed.data, editor.revision)); }}>Review changes</button><button type="button" disabled={busy} onClick={() => setEditor(null)}>discard</button></div>
    </div>}
    {review && compiledReview && <section className="plugin-atyrode_code_generator__catalog-review" aria-label="Catalog review">
      ...
      <div className="plugin-atyrode_code__toolbar"><button type="button" className="plugin-atyrode_code__primary-action" data-action="atyrode.code.promoteCatalog" disabled={busy || !currentReview} onClick={() => void perform(async () => { await callCodeAction(host, "promoteCatalog", { containerId: host.containerId!, expectedRevision: review.revision, source: review.source, reviewDigest: review.reviewDigest }); if (mounted.current) onDone(); })}>Use this catalog</button><button type="button" disabled={busy} onClick={() => { if (reviewDocument) edit(reviewDocument); }}>edit models</button><button type="button" disabled={busy} onClick={() => setReview(null)}>back</button></div>
