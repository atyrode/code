...
func operationArgs(operation, payload string) ([]string, *int64, error) {
	...
	case "account-set":
		...
		if decodeDocument(data, &p, true) != nil || p.Provider == "" || p.Identity == "" {
			return nil, nil, errInvalid
		}
	...
	case "preset-create", "preset-update":
		...
		if decodeDocument(data, &p, true) != nil || strings.TrimSpace(p.Name) == "" {
			return nil, nil, errInvalid
		}
		for _, reference := range p.Disabled {
			if reference.Provider == "" || reference.IdentityKey == "" {
				return nil, nil, errInvalid
			}
		}
